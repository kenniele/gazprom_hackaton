...
#  В эту функцию передается словарь типа {"role": any, "content": any}, и функция превращает ее в
#  объект Message, который обрабатывается моделью Mistral AI
    ...
    async def acompletion_with_retry(
        ...
        async def _completion_with_retry(**kwargs: Any) -> Any:
            try:
                request_timeout = kwargs.pop("request_timeout")
                request = Requests(headers=self._headers())
                async with request.apost(url=self._url(),
                                         data=self._body(kwargs),
